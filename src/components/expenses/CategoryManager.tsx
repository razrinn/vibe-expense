...
const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  const handleSaveCategory = () => {
    if (!validateCategoryName(newCategoryName, categories)) {
      setError(newCategoryName.trim() ? 'Category name already exists' : 'Category name cannot be empty');
      return;
    }
  ...
  const handleUpdateCategory = (id: string) => {
    if (!validateCategoryName(editName, categories, id)) {
      setError(editName.trim() ? 'Category name already exists' : 'Category name cannot be empty');
      return;
    }
      ...
      {isAddingCategory && (
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-md">
          <div className="space-y-3">
            ...
            <div>
              <label htmlFor="newCategoryColor" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Color
              </label>
              <div className="mt-1 flex items-center space-x-2">
                ...
                <input
                  ...
                  value={newCategoryColor}
                  onChange={(e) => setNewCategoryColor(e.target.value)}
                  className="block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-green-500 focus:ring-green-500 dark:bg-gray-800 dark:text-white"
                  placeholder="#HEX"
                />
              </div>
            </div>
            <div className="flex justify-end space-x-2 mt-3">
              <button
                ...
                className="inline-flex items-center px-3 py-1 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
              >
          ...
          {categories.map(category => (
            <li key={category.id} className="p-4">
              {editingCategoryId === category.id ? (
                <div className="space-y-3">
                  <div>
                    <label htmlFor={`edit-${category.id}-name`} className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                      Category Name
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label htmlFor={`edit-${category.id}-color`} className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                      Color
                    </label>
                    <div className="mt-1 flex items-center space-x-2">
                      <input
